...
void CLine::apply()
{
	...
	m_normalCount = MH_MAX(m_pointArr.size() - 1, 0);
	...
	{
		v.x = m_pointArr[i - 1].x - m_pointArr[i].x;
		v.y = m_pointArr[i - 1].y - m_pointArr[i].y;
		v.normalize();
		float tmpx = v.x;
		...
		v.x = v.x * m_normalScale;
		v.y = v.y * m_normalScale;
