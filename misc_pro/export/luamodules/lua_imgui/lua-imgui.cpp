...
static int
ioLoadFont(lua_State *L){
	ImGuiIO& io = ImGui::GetIO();
	const char *path = luaL_checkstring(L,1);
	float size = (float)luaL_checknumber(L,2);
	ImFont *font = io.Fonts->AddFontFromFileTTF(path,size,NULL,io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
	return 0;
}

static int
ioSetDisplaySize(lua_State *L){
	ImGuiIO& io = ImGui::GetIO();
...
static int
ioMouseState(lua_State *L){
	int button = luaL_checkinteger(L,1);
	int press = luaL_checkinteger(L,2);
	ImGuiIO& io = ImGui::GetIO();
...
luaopen_imgui(lua_State *L) {
	...
	luaL_Reg l[] = {
		{ "Create",lcreate},
		{ "NewFrame",lnewframe},
		{ "SetMousePos",ioSetMousePos},
		{ "GetMousePos",ioGetMousePos},
		{ "WantSetMousePos",ioWantSetMousePos},
		{ "SetMouseWheel",ioSetMouseWheel},
		{ "SetDisplaySize",ioSetDisplaySize},
		{ "MouseState",ioMouseState},
		{ "LoadFont",ioLoadFont},
		{ "KeyMap", lkeymap },
		{ "KeyState", lkeyState },
		{ "InputChar", linputChar },
		{ NULL, NULL },
	};
